...
function computeEngineVersionLogic(history, version) {
    ...
    if (version === "PRIVATE_GOD_MODE") {
        ...
        if (rushDiff >= 4 || streak >= 3) {
            rawSize = sizes[0] === 'BIG' ? 'SMALL' : 'BIG';
            baseConf = 99;
            strategyDesc = "🔒 PRIVATE GOD MODE (Inverted Rush Call)";
        } else {
            return { prediction: "SKIP", confidence: "SAFE", method: "🔒 PRIVATE GOD MODE - Waiting" };
        }
        ...
        rawSize = (streak >= 3) ? (sizes[0] === 'BIG' ? 'SMALL' : 'BIG') : 'BIG';
        baseConf = 95;
        strategyDesc = "🔥 AI PRO - Diablo Original";
    }

    // INVERSE LOGIC: Big ko Small aur Small ko Big banana
    let targetSize = rawSize === "BIG" ? "SMALL" : (rawSize === "SMALL" ? "BIG" : "SKIP");

    if (baseConf >= 95 && targetSize !== "SKIP") {
        lastSentPrediction = targetSize;
        sendTelegramAlert(`🚨 *DIABLO-Z ${version} (INVERTED)* 🚨\n\n🎯 *Prediction:* \`${targetSize}\`\n📊 *Conf:* \`${baseConf}%\`\n⚙️ *Strat:* ${strategyDesc}`);
    }
